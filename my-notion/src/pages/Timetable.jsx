...
export default function Timetable() {
    ...
    const downloadPDF = () => {
        try {
            ...
            doc.text('Generated by MyNotion', 14, 30)

            const gridData = []
            DAYS.forEach(d => {
                const rowData = [d.name]
                SLOTS.forEach(s => {
                    if (s.id === 2) {
                        ...
                        if (assignedSubject) {
                            let cellText = assignedSubject.name
                            if (roomNumber) {
                                cellText += `\nRoom: ${roomNumber}`
                            }
            ...
            autoTable(doc, {
                startY: 40,
                head: [['Day', ...SLOTS.map(s => s.time)]],
                body: gridData,
                theme: 'grid',
                styles: { fontSize: 9, cellPadding: 4, valign: 'middle', halign: 'center' },
                headStyles: { fillColor: [99, 102, 241], textColor: 255 },
                columnStyles: {
                    0: { fontStyle: 'bold', fillColor: [248, 249, 250], textColor: [0, 0, 0], halign: 'left' }
                },
                margin: { top: 40 }
            })
